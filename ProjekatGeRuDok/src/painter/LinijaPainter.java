...
public class LinijaPainter extends ElementPainter {

	public LinijaPainter(Element device) {
		super(device);
//		LinijaElement linija = (LinijaElement) device;
//		
//		shape = new GeneralPath();
//		((GeneralPath)shape).moveTo(linija.getOutput().getPosition().getX() + 5, linija.getOutput().getPosition().getY());
//		((GeneralPath)shape).lineTo(linija.getInput().getPosition().getX() - 5, linija.getInput().getPosition().getY());
		
	}
	
	public static Rectangle findRectangle(LinijaElement link){
