...
public class SearchOR implements ActionListener {

	private JDialog dialog;
	
	public SearchOR(JDialog dialog) {
		this.dialog = dialog;
	}
		...
		for(int i = 0; i < atr; i++) {
			...
			if(!txt.equals("")) {
				...
				if(prvi) {
					...
					if(atribut.getType().equals("char") || atribut.getType().equals("varchar")) {
						findSQL += " OR " + atribut.getName() + " LIKE  ? ";
					} else {
						findSQL += " OR " + atribut.getName() + " " + txt.substring(0, 1) + " ? ";
					}
				}
			}
		}
