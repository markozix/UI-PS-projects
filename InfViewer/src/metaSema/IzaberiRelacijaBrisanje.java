...
import controller.IzaberiRelacijaBrisanjeAkcija;
...
public class IzaberiRelacijaBrisanje extends JDialog {

	private JList<Relacija> lista;
	private DefaultListModel<Relacija> model;
	
	public IzaberiRelacijaBrisanje() {
		model = new DefaultListModel<>();
		lista = new JList<>(model);
		setTitle("Izaberite relaciju koji se brise");
		setSize(300, 300);
		...
		for(int i = 0; i < resurs.getChildCount(); i++) {
			...
			for(int k = 0; k < p.getChildCount(); k++) {
				Entitet e = (Entitet)resurs.getChildAt(k);
		...
		dugme.addActionListener(new IzaberiRelacijaBrisanjeAkcija(this));
